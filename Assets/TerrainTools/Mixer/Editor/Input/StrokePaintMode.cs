...
namespace Rowlan.TerrainTools.Mixer
{
    public class StrokePaintMode : IPaintMode
    {
        ...
        public void OnInspectorGUI(Terrain terrain, IOnInspectorGUI editContext, BrushSettings brushSettings)
        {
            ...
            if (m_StartTerrain == null || m_StartPoint == null)
            {
                EditorGUILayout.HelpBox(MixerStyles.anchorRequiredContent.text, MessageType.Warning);
            }

            EditorGUILayout.HelpBox(MixerStyles.strokeUsageContent.text, MessageType.None);

            jitterProfile = EditorGUILayout.CurveField(MixerStyles.jitterProfileContent, jitterProfile);

            m_Spacing = EditorGUILayout.Slider(MixerStyles.splatDistanceContent, m_Spacing, 1.0f, 100.0f);
        }
